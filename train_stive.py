...
from stive.models.tp_unet import UNet3DConditionModel
from stive.data.dataset import TGVEDataset, TGVEValDataset
from stive.pipelines.pipeline_stive import STIVEPipeline

from stive.util import save_videos_grid, save_video, pad_to_tensor, ddim_inversion
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    validation_pipeline = STIVEPipeline(
                        ...
                        for name, module in unet.named_modules():
                            if name.endswith(tuple(trainable_modules)):
                                for param in module.parameters():
                                    ...
                                    # else:
                                    #     print(f"Epoch {epoch}, Parameter: {name}, min Gradient: {param.grad.abs().min().item()}, Requires_grad: {param.requires_grad}")
    ...
    if accelerator.is_main_process:
        ...
        pipeline = STIVEPipeline.from_pretrained(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/stive.yaml")
