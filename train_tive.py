...
from tive.models.abstracts_clip import AbstractsCLIPTextModel, AbstractsCLIPTokenizer
from tive.models.unet import UNet3DConditionModel

from tive.data.dataset import TGVEDataset, TGVEValDataset
from tive.pipelines.pipeline_tive import TIVEPipeline
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    tokenizer = AbstractsCLIPTokenizer.from_pretrained(pretrained_model_path, subfolder="tokenizer", abstracts_list=abstracts_list, abstracts_num_embedding=abstracts_num_embedding)
    text_encoder = AbstractsCLIPTextModel.from_pretrained(pretrained_model_path, subfolder="text_encoder", abstracts_list=abstracts_list, abstracts_num_embedding=abstracts_num_embedding)
    ...
    validation_pipeline = TIVEPipeline(
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("tive-fine-tune")
            ...
            for step, batch in enumerate(progress_bar):
                with accelerator.accumulate(unet):
                    ...
                    if hasattr(tokens, 'replace_indices') and hasattr(tokens, 'abstract_indices'):
                        ...
                        print(tokens.replace_indices)
                        print(tokens.abstract_indices)
                    ...
                    if accelerator.sync_gradients:
                        accelerator.clip_grad_norm_(list(unet.parameters())+list(text_encoder.parameters()), max_grad_norm)
                        for name, module in unet.named_modules():
                            if name.endswith(tuple(trainable_modules)):
                    ...
                    logs = {"Epoch": f'{epoch}/{num_train_epoch}', "step_loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
    ...
    if accelerator.is_main_process:
        ...
        pipeline = TIVEPipeline.from_pretrained(
