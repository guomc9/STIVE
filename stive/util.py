...
def get_noise_pred_single(latents, t, context, condition, unet):
    noise_pred = unet(latents, t, encoder_hidden_states=context, condition=condition)["sample"]
    return noise_pred


@torch.no_grad()
def ddim_loop(pipeline, ddim_scheduler, latent, num_inv_steps, prompt, condition, unet_2d=False):
    f = latent.shape[2]
    context = init_prompt(prompt, pipeline)
    uncond_embeddings, cond_embeddings = context.chunk(2)
    if unet_2d:
        cond_embeddings = repeat(cond_embeddings, 'b n c -> (b f) n c', f=f)
    ...
    for i in tqdm(range(num_inv_steps)):
        ...
        noise_pred = get_noise_pred_single(latent, t, cond_embeddings, condition, pipeline.unet)
        if unet_2d:
            latent = next_step(noise_pred, t, rearrange(latent, 'b c f h w -> (b f) c h w'), ddim_scheduler)
...
@torch.no_grad()
def ddim_inversion(pipeline, ddim_scheduler, video_latent, num_inv_steps, prompt="", condition=None,  unet_2d=False):
    ddim_latents = ddim_loop(pipeline, ddim_scheduler, video_latent, num_inv_steps, prompt, condition, unet_2d)
    return ddim_latents
