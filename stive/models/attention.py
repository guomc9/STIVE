...
from diffusers.models.attention import Attention, GatedSelfAttentionDense, _chunked_feed_forward
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if is_xformers_available():
            self.attn1.processor = SparseCausalXFormersAttnProcessor()
        else:
            self.attn1.processor = SparseCausalAttnProcessor2_0()
    ...
    def forward(
        ...
        self.attn1.video_length = video_length
        ...
        if self.attn2 is not None:
            self.attn2.video_length = video_length
            if self.norm_type == "ada_norm":
...
class SparseCausalXFormersAttnProcessor:
    r"""
    Processor for implementing memory efficient sparse causal attention using xFormers.
...
class SparseCausalAttnProcessor2_0:
    r"""
    Processor for implementing scaled dot-product attention (enabled by default if you're using PyTorch 2.0).
    """
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("SparseCausalAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        hidden_states = attn._attention(query, key, value, attention_mask)
