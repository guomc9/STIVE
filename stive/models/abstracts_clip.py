...
class AbstractsCLIPTokenizer(CLIPTokenizer):
    ...
    def __call__(self, text, prefix: str=None, **kwargs):
        ...
        print(f'all_abstract_indices: {all_abstract_indices}')
    ...
    @classmethod
    def from_pretrained_clip(cls, pretrained_model_name_or_path, *model_args, abstracts_list:list[str]=None, abstracts_num_embedding:int=1,**kwargs):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        abstracts_path = os.path.join(pretrained_model_name_or_path, "abstracts.json")
        if os.path.exists(abstracts_path):
            with open(abstracts_path, 'r', encoding='utf-8') as f:
                abstracts = json.load(f)
                model.abstracts_list = abstracts['abstracts_list']
                model.abstracts_num_embedding = abstracts['abstracts_num_embedding']
    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        abstracts_path = os.path.join(save_directory, "abstracts.json")
        with open(abstracts_path, 'w', encoding='utf-8') as f:
            json.dump(
                {
                    'abstracts_list': self.abstracts_list, 
                    'abstracts_num_embedding': self.abstracts_num_embedding
                }, 
...
class AbstractsCLIPTextModel(CLIPTextModel):
        ...
        if self.abstracts_list is not None and replace_indices is not None and abstract_indices is not None:
            ...
            for i in range(len(replace_indices)):
                ...
                if self.retain_position_embedding:
                    hidden_states[i, replace_idx] = self.abstracts_embedder(abstract_idx) + self.text_model.embeddings.position_embedding(replace_idx)
                else:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if subfolder is not None:
            abstracts_path = os.path.join(pretrained_model_name_or_path, subfolder, 'abstracts.json')
            abstracts_weight_path = os.path.join(pretrained_model_name_or_path, subfolder, 'abstracts_embedder.pth')
        else:
            abstracts_path = os.path.join(pretrained_model_name_or_path, 'abstracts.json')
            abstracts_weight_path = os.path.join(pretrained_model_name_or_path, 'abstracts_embedder.pth')
        ...
        if os.path.exists(abstracts_path):
            with open(abstracts_path, 'r', encoding='utf-8') as f:
                ...
                model.embedding_dim = config.hidden_size
                model.abstracts_num_embedding = abstracts['abstracts_num_embedding']
                model.retain_position_embedding = abstracts['retain_position_embedding']
                model.abstracts_embedder = nn.Embedding(num_embeddings=len(model.abstracts_list) * model.abstracts_num_embedding, embedding_dim=model.embedding_dim)

        model.abstracts_embedder.load_state_dict(torch.load(abstracts_weight_path))
        print('AbstractsCLIPTextModel.abstracts_embedder weights loaded.')
        
        return model

    def save_pretrained(self, save_directory, **kwargs):
        super().save_pretrained(save_directory, **kwargs)
        abstracts_embedder_path = os.path.join(save_directory, "abstracts_embedder.pth")

        torch.save(self.abstracts_embedder.state_dict(), abstracts_embedder_path)
...
def abstracts_contrastive_loss(abstracts_tokenizer: AbstractsCLIPTokenizer, abstracts_text_encoder: AbstractsCLIPTextModel, prefix: str = None, reduction:str = None):
    if (len(abstracts_tokenizer.abstracts_list)) > 1:
        prompts = []
        for i, abstract in enumerate(abstracts_tokenizer.abstracts_list):
            if prefix is not None:
        ...
        abstracts_tokens = abstracts_tokenizer(prompts, return_tensors="pt", max_length=abstracts_tokenizer.model_max_length, padding="max_length", truncation=True)
        abstracts_feat = abstracts_text_encoder(abstracts_tokens.input_ids.to(abstracts_text_encoder.device), abstracts_tokens.replace_indices, abstracts_tokens.abstract_indices)[1]   # [B, C]
        # TODO
        abstracts_feat = torch.nn.functional.normalize(abstracts_feat, p=2, dim=1)
        cos_sim_matrix = torch.mm(abstracts_feat, abstracts_feat.t())                               # [B, B]
        batch_size = cos_sim_matrix.size(0)
        negs = cos_sim_matrix[~torch.eye(batch_size, dtype=bool, device=cos_sim_matrix.device)]     # [B, B]
        return -torch.mean(negs) if reduction is None or reduction == 'mean' else -torch.sum(negs)

    else:
        return torch.Tensor(0.0, device=abstracts_text_encoder.device, dtype=torch.float)