...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    # Load VAE
    vae = AutoencoderKL3D.from_pretrained_2d(pretrained_model_path, subfolder="vae", num_temp_down_blocks=num_temp_down_blocks, num_temp_up_blocks=num_temp_up_blocks)
    # torch.nn.MSELoss()
            ...
            for step, batch in enumerate(progress_bar):
                with accelerator.accumulate(vae):
                    ...
                    with accelerator.autocast():
                        mid = vae.encode(source, video_length)
                    
                        z = mid.latent_dist.sample()
                        z_td = mid.latent_dist_td.sample()
                        td_seq = mid.td_seq
                        pred = vae.decode(z, video_length=video_length, z_td=z_td, td_seq_len=td_seq.shape[0]).sample
                        rec_loss = F.mse_loss(pred.float(), source.float(), reduction="mean")
                        max_diff = torch.max((pred.float()-source.float()).abs())
                ...
                logs = {"Epoch": f'{epoch}/{num_train_epoch}', "Step": f'{global_step}/{num_train_steps}', "step_loss": loss.detach().item(), "rec_loss": rec_loss.detach().item(), "kl_loss": kl_loss.detach().item(), "max_diff": 127.5 * max_diff.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
