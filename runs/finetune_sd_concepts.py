...
from stive.models.attention import reset_sparse_casual_processors
...
def log_cross_attention(accelerator, prompt_attn_dict, save_path, step, is_src_cross_attn: bool):
    ...
    for prompt, unet_attns in prompt_attn_dict.items():
        for key, unet_attn in unet_attns.items():
            ...
            accelerator.log({f"{key}/{'source' if is_src_cross_attn else 'target'}": wandb.Video(unet_attn.to(torch.uint8).detach().cpu().numpy())}, step=step)
    return
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="stive-sd-concepts-fine-tune", 
            config={
                ...
                'supvis_conf': {
                        ...
                        "enable_tcam_loss": enable_tcam_loss, 
    ...
    tokenizer = ConceptsCLIPTokenizer.from_pretrained_clip(pretrained_sd_model_path, subfolder="tokenizer", concepts_list=concepts, concepts_num_embedding=concepts_num_embedding)
    text_encoder = ConceptsCLIPTextModel.from_pretrained_clip(pretrained_sd_model_path, subfolder="text_encoder", concepts_list=concepts, concepts_num_embedding=concepts_num_embedding, retain_position_embedding=retain_position_embedding)
    ...
    for name, param in lora_unet.named_parameters():
        if 'lora' in name and 'temp_lora_conv' not in name:
    ...
    optimizer = optimizer_cls(
        [
            {"params": text_encoder.concepts_embedder.weight, "lr": learning_rate}, 
            {"params": [p for p in lora_unet.parameters() if p.requires_grad], "lr": lora_lr}
        ],
        lr=learning_rate,
    ...
    clip_tokenizer = CLIPTokenizer.from_pretrained(pretrained_sd_model_path, subfolder="tokenizer")
    clip_text_encoder = CLIPTextModel.from_pretrained(pretrained_sd_model_path, subfolder="text_encoder")
    ...
    validation_pipeline = TextToVideoSDPipeline.from_pretrained(
        pretrained_model_name_or_path=pretrained_sd_model_path, vae=vae, text_encoder=clip_text_encoder, tokenizer=clip_tokenizer, unet=lora_unet, 
    )
    ...
    optim_params.append(text_encoder.concepts_embedder.weight)
    text_encoder, lora_unet, optimizer, train_dataloader, val_dataloader, lr_scheduler = accelerator.prepare(
        text_encoder, lora_unet, optimizer, train_dataloader, val_dataloader, lr_scheduler
    )
    ...
    if enable_scam_loss or enable_tcam_loss:
        ...
        register_attention_control(lora_unet, supervisor, begin_store=unet_begin_store_idx, only_cross=True, replace_attn_prob=False, self_to_st_attn=False)
        ...
        with tqdm(train_dataloader) as progress_bar:
            for step, batch in enumerate(progress_bar):
                    ...
                    with accelerator.accumulate(text_encoder):
                        ...
                        if enable_scam_loss and tokens.replace_indices is not None:
                            ...
                            if global_step % attn_check_steps == 0:
                                ...
                                log_cross_attention(accelerator, prompt_attn_dict, save_path, step=global_step, is_src_cross_attn=True)
                        ...
                        if enable_tcam_loss and batch.get('target_latents') is not None and batch.get('target_masks') is not None:
                            ...
                            target_tokens = tokenizer(target_prompts, enable_prefix=False, return_tensors="pt", max_length=tokenizer.model_max_length, padding="max_length", truncation=True)
                            target_encoder_hidden_states = text_encoder(target_tokens.input_ids.to(latents.device), target_tokens.replace_indices, target_tokens.concept_indices)[0]
                            if target_tokens.replace_indices is not None:
                                ...
                                if global_step % attn_check_steps == 0:
                                    ...
                                    log_cross_attention(accelerator, prompt_attn_dict, save_path, step=global_step, is_src_cross_attn=False)
                        ...
                        if accelerator.sync_gradients:
                            ...
                            for param in optim_params:
                                if param.requires_grad and param.grad is None:
                                        print(f"Epoch {epoch}, Parameter: {param}, Grad: {param.grad}, Shape: {param.shape}, Requires_grad: {param.requires_grad}")
                ...
                if global_step % validation_steps == 0:
                    ...
                    if enable_scam_loss and enable_tcam_loss:
                        supervisor = StepAttentionSupervisor()
                        register_attention_control(lora_unet, supervisor, begin_store=unet_begin_store_idx, only_cross=True, replace_attn_prob=False)
