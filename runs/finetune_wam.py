...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    validation_pipeline = TextToVideoSDPipeline.from_pretrained(
        pretrained_model_name_or_path=pretrained_t2v_model_path, vae=vae, text_encoder=clip_text_encoder, tokenizer=clip_tokenizer, unet=lora_unet, 
    )
        ...
        with tqdm(train_dataloader) as progress_bar:
            for step, batch in enumerate(progress_bar):
                with accelerator.autocast():
                    with accelerator.accumulate(text_encoder, lora_unet):
