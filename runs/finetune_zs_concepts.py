    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="stive-zs-concepts-finetune", 
            config={
    ...
    tokenizer = ConceptsCLIPTokenizer.from_pretrained_clip(pretrained_zs_model_path, subfolder="tokenizer", concepts_list=concepts, concepts_num_embedding=concepts_num_embedding)
    text_encoder = ConceptsCLIPTextModel.from_pretrained_clip(pretrained_zs_model_path, subfolder="text_encoder", concepts_list=concepts, concepts_num_embedding=concepts_num_embedding, retain_position_embedding=retain_position_embedding)
    ...
    clip_tokenizer = CLIPTokenizer.from_pretrained(pretrained_zs_model_path, subfolder="tokenizer")
    clip_text_encoder = CLIPTextModel.from_pretrained(pretrained_zs_model_path, subfolder="text_encoder")
    ...
    validation_pipeline = TextToVideoSDPipeline.from_pretrained(
        pretrained_model_name_or_path=pretrained_zs_model_path, vae=vae, text_encoder=clip_text_encoder, tokenizer=clip_tokenizer, unet=unet, 
    )
